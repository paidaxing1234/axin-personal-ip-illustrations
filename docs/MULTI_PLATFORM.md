...
| 平台 | 入口文件 | 安装方式 | 状态 |
| --- | --- | --- | --- |
| Codex | `amo-personal-ip-illustrations/SKILL.md` | `scripts/install-local-skill.ps1` | 已支持 |
| Hermes | 同一份 `SKILL.md` | `scripts/install-hermes-skill.ps1` | 已支持 |
| Claude Code | `CLAUDE.md` / `.claude-plugin/` | 作为 repo 上下文或插件包 | 已支持 |
| Cursor | `.cursor/rules/amo-personal-ip-illustrations.mdc` | 自动读取 rules | 已支持 |
| Windsurf | `.windsurfrules` | 自动读取 rules | 已支持 |
| Cline | `.clinerules/amo-personal-ip-illustrations.md` | 自动读取 rules | 已支持 |
...
```text
C:\Users\<you>\.codex\skills\amo-personal-ip-illustrations
```
...
```text
$HERMES_HOME\skills\creative\amo-personal-ip-illustrations
```

使用：

```bash
hermes -s amo-personal-ip-illustrations
```
...
```text
Read llms.txt first. Then read amo-personal-ip-illustrations/SKILL.md.
