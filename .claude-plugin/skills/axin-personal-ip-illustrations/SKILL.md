...
3. 如果当前 agent 环境提供图片生成工具，例如 `image_gen`、`imagegen`、内置生图能力或其他可调用的图像生成工具，就直接逐张生成图片。
...
当用户提到“抖音图文 / 小红书图文 / 轮播图 / 竖屏图文 / 9:16 / 图文作品”时，默认切换到竖屏轮播模式，而不是 16:9 正文配图。

竖屏轮播默认规则：

- 画幅使用 9:16 竖屏，优先 `1080x1920` 或当前 `image_gen` 支持的等比例竖图尺寸。
- 默认做 5-8 张：1 张封面、3-5 张过程卡、1 张结尾行动卡。
- 每张图只表达一个观点，保留大面积留白，适合后期叠加中文大字。
- 如果当前环境暴露 `image_gen`，必须直接逐张调用 `image_gen` 生成图片，不要退回 CLI、本地绘图或只输出 prompt。
- 如果 `image_gen` 没有暴露，才输出逐张完整 prompt，并明确说明“当前环境未暴露可调用的 image_gen”。
- 中文大标题要短，建议每页 6-14 个字；复杂中文不要强迫生图模型一次画完，优先让图承担画面，标题可后期排版。
- 保存路径使用 `assets/<topic-slug>-douyin-carousel/`，命名为 `01-cover.png`、`02-problem.png` 等。
